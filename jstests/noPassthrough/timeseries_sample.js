...
let assertPlanForSample = (explainRes, backupPlanSelected) => {
    if (backupPlanSelected) {
        assert.neq(getPlanStage(getWinningPlan(explainRes.stages[0].$cursor.queryPlanner),
                                "UNPACK_BUCKET"),
                   null);
        assert(!aggPlanHasStage(explainRes, "$_internalUnpackBucket"));
        assert(aggPlanHasStage(explainRes, "$sample"));
    } else {
        assert.neq(getPlanStage(getWinningPlan(explainRes.queryPlanner), "QUEUED_DATA"));
        assert(!aggPlanHasStage(explainRes, "$_internalUnpackBucket"));
        assert(!aggPlanHasStage(explainRes, "$sample"));
    }
};
...
// Use a measurementsPerBucket of 1 to exercise the fallback branch of the TrialStage.
runSampleTests(1, true);

// Use a measurementsPerBucket of 950 to exercise the trial branch of the TrialStage.
runSampleTests(950, false);

// Check that $sample hasn't been absorbed by $_internalUnpackBucket when the sample size is
// sufficiently large and we don't attempt the TrialStage
let sampleSize = 101;
...
assert.eq(sampleSize, unoptimizedResult.length, unoptimizedResult);
