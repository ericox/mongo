...
createRandomCursorExecutor(const CollectionPtr& coll,
                           ...
                           boost::optional<BucketUnpacker> bucketUnpacker = boost::none) {
    ...
    if (collectionFilter.isSharded() && !expCtx->ns.isTimeseriesBucketsCollection()) {
        ...
        // UnpackTimeseriesBucket with $sample left in the pipeline in-place. Using
        // UnpackTimeseriesBucket as the backup plan will fallback to using the top-k sort sampling
        // approach. Suppose the 'gTimeseriesBucketMaxCount' is 1000, but each bucket only contains
        // 500 documents on average. The observed trial advanced/work ratio approximates the average
        // bucket fullness (abf) = 500 / 1000 = 0.5. Experiments have shown that the optimized
        // SampleFromTimeseriesBucket algorithm performs better than backup plan when the sampleSize
        // < 0.02 * abf * 'numRecords' * 'gTimeseriesBucketMaxCount'.
        static const auto kAvgBucketFillSampleFraction = 0.02;
        static const auto kAvgBucketFill = 0.5;
        const auto minAdvancedToWorkRatio = std::max(
            std::max(sampleSize /
                         (kAvgBucketFillSampleFraction * numRecords * gTimeseriesBucketMaxCount),
                     1.0),
            kAvgBucketFill);
        ...
        auto arhashPlan = std::make_unique<SampleFromTimeseriesBucket>(
            ...
            std::move(root),
            *bucketUnpacker,
            gTimeseriesSampleMaxNonDuplicateTriesBeforeFail +
                10 /* don't fail during de-dupe attempts */,
            sampleSize,
