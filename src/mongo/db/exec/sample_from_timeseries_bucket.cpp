...
namespace mongo {
const char* SampleFromTimeseriesBucket::kStageType = "STAGE_SAMPLE_FROM_TIMESERIES_BUCKET";
...
SampleFromTimeseriesBucket::SampleFromTimeseriesBucket(ExpressionContext* expCtx,
    ...
    : PlanStage{kStageType, expCtx},
      ...
      _worksSinceLastAdvanced{worksSinceLastAdvanced},
...
PlanStage::StageState SampleFromTimeseriesBucket::doWork(WorkingSetID* out) {
    ...
    if (PlanStage::ADVANCED == status) {
        ...
        uassert(5521504,
                str::stream() << kStageType
                              << " could not find a non-duplicate measurement after 100 attempts",
                _worksSinceLastAdvanced < 100);
