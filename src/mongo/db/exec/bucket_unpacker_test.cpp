...
public:
    ...
    BucketUnpacker makeBucketUnpacker(std::set<std::string> fields,
                                      ...
                                      boost::optional<int> errorCode = boost::none) {

        auto spec = BucketSpec{kUserDefinedTimeName.toString(), metaFieldName, std::move(fields)};

        auto unpacker =
            BucketUnpacker{std::move(spec), behavior, includeTimeField, includeMetaField};
        if (!errorCode) {
...
TEST_F(BucketUnpackerTest, UnpackBasicIncludeWithDollarPrefix) {
    std::set<std::string> fields{"_id", "time", "$a", "b"};
...
TEST_F(BucketUnpackerTest, MissingMetaFieldDoesntMateralizeMetadata) {
    std::set<std::string> fields{};
...
TEST_F(BucketUnpackerTest, ExcludedMetaFieldDoesntMateralizeMetadataWhenBucketHasMeta) {
...
TEST_F(BucketUnpackerTest, UnpackerResetThrowsOnUndefinedMeta) {
    ...
    auto unpacker = makeBucketUnpacker(std::move(fields),
                                       ...
                                       std::move(bucket),
                                       kUserDefinedMetaName.toString(),
                                       5369600 /* reset() should throw this code */);
}
...
TEST_F(BucketUnpackerTest, UnpackerResetThrowsOnUnexpectedMeta) {
    ...
    auto unpacker = makeBucketUnpacker(std::move(fields),
                                       ...
                                       5369601 /* reset() should throw this code */);
}

TEST_F(BucketUnpackerTest, NullMetaInBucketMateralizesAsNull) {
    std::set<std::string> fields{};
...
TEST_F(BucketUnpackerTest, EmptyDataRegionInBucketDoesnt) {
    std::set<std::string> fields{};
...
TEST_F(BucketUnpackerTest, UnpackerResetThrowsOnEmptyBucket) {
    ...
    auto unpacker = makeBucketUnpacker(std::move(fields),
                                       ...
                                       bucket.toBson(),
                                       kUserDefinedMetaName.toString(),
                                       5346510 /* reset() should throw this code */);
}
